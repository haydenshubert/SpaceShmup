...
public class EnemyShield : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        blinker.ignoreOnCollisionEnter = true; // error for now
    ...
    /// <summary>
    /// Called by another EnemyShield to join th eprotectors of this EnemyShield
    /// </summary>
    ...
    /// <summary>
    /// Called by Enemy_4.OnCollisionEnter() & parent's EnemyShileds.TakeDamage()
    /// to distribute damage to EnemyShield protectors
    /// </summary>
    /// <param name="dmg">The amount of damage to be handled</param>
    /// <returns>Any damage not handled by this shiedl</returns>
    public float TakeDamage (float dmg)
    {
        // Can we pass damage to a protector EnemyShiedl?
        foreach (EnemyShield es in protectors)
        {
            if (es.isActive)
            {
                dmg = es.TakeDamage(dmg);
                // If all damage was handeld, return 0 damage
                if (dmg == 0) return 0;
            }
        }
